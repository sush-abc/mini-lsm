...
impl SsTable {
    ...
    fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        ...
        let start_offset = meta.offset as u64;
        let end_offset = if block_idx + 1 < self.block_meta.len() {
            self.block_meta[block_idx + 1].offset as u64
        } else {
            self.block_meta_offset as u64
        };
        let serialized_block = self.file.read(start_offset, end_offset - start_offset)?;
        Ok(Arc::new(Block::decode(&serialized_block)))
    }
